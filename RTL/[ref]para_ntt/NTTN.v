...
always @(posedge clk or posedge reset) begin
    if(reset) begin
        ...
        3'd0: begin                 // 0: IDLE
            if(load_w)
                state <= 3'd1;      // load twiddle factors + q + n_inv
            else if(load_data)
                state <= 3'd2;      // load data
            else if(start | start_intt)
                state <= 3'd3;      // performs ntt
            else
                state <= 3'd0;      // IDLE
            sys_cntr <= 0;
        end
        3'd1: begin                 // 1: load twiddle factors + q + n_inv
        ...
        3'd3: begin                 // 3: performs ntt
            if(ntt_finished && (ntt_intt == 0))
                state <= 3'd4;      // output data
            else if(ntt_finished && (ntt_intt == 1))
                state <= 3'd5;      // last stage of intt
            else
                state <= 3'd3;      // performs ntt
            sys_cntr <= 0;
        end
        3'd4: begin                 // 4: output data
            if(sys_cntr == (`RING_SIZE+1)) begin
                state <= 3'd0;      // IDLE
                sys_cntr <= 0;
            end
        ...
        3'd5: begin                 // 5: last stage of intt
