...
def NTT8(arrayIn, q, phi):
    ...
    for i in range(32768):
        a0[i]       = (arrayIn[i] + arrayIn[i+32768]) % q
        a0[i+32768] = (arrayIn[i] - arrayIn[i+32768]) * (phi**i % q) % q
    ...
    for i in range(16384):
        a1[i]       = (a0[i]       + a0[i+16384]) % q
        a1[i+16384] = (a0[i]       - a0[i+16384]) * (phi**(2*i) % q) % q
        a1[i+32768] = (a0[i+32768] + a0[i+49152]) % q
        a1[i+49152] = (a0[i+32768] - a0[i+49152]) * (phi**(2*i) % q) % q
    ...
    print("NTT-STAGE-2")
    for i in range(8192):
        a2[i]       = (a1[i]       + a1[i+8192])  % q
        a2[i+8192]  = (a1[i]       - a1[i+8192])  * (phi**(4*i) % q) % q
        a2[i+16384] = (a1[i+16384] + a1[i+24576]) % q
        a2[i+24576] = (a1[i+16384] - a1[i+24576]) * (phi**(4*i) % q) % q
        a2[i+32768] = (a1[i+32768] + a1[i+40960]) % q
        a2[i+40960] = (a1[i+32768] - a1[i+40960]) * (phi**(4*i) % q) % q
        a2[i+49152] = (a1[i+49152] + a1[i+57344]) % q
        a2[i+57344] = (a1[i+49152] - a1[i+57344]) * (phi**(4*i) % q) % q
    
    # Stage 3
    ### group 0: {0,4096},      {1,4097},      ..., {4095,8191}
    ### group 1: {8192,12288},  {8193,12289},  ..., {12287,16383}
    ### group 2: {16384,20480}, {16385,20481}, ..., {20479,24575}
    ### ...
    ### group 7: {57344,61440}, {57345,61441}, ..., {61439,65535}  
    print("NTT-STAGE-3")
    for j in range(8):
        for i in range(4096):
            a3[i+j*8192]      = (a2[i+j*8192] + a2[i+j*8192+4096]) % q
            a3[i+j*8192+4096] = (a2[i+j*8192] - a2[i+j*8192+4096]) * (phi**(8*i) % q) % q
    ...
    for j in range(16):
        for i in range(2048):
            a4[i+j*4096]      = (a3[i+j*4096] + a3[i+j*4096+2048]) % q
            a4[i+j*4096+2048] = (a3[i+j*4096] - a3[i+j*4096+2048]) * (phi**(16*i) % q) % q
    ...
    for j in range(32):
        for i in range(1024):
            a5[i+j*2048]      = (a4[i+j*2048] + a4[i+j*2048+1024]) % q
            a5[i+j*2048+1024] = (a4[i+j*2048] - a4[i+j*2048+1024]) * (phi**(32*i) % q) % q
    ...
    for j in range(64):
        for i in range(512):
            a6[i+j*1024]     = (a5[i+j*1024] + a5[i+j*1024+512]) % q
            a6[i+j*1024+512] = (a5[i+j*1024] - a5[i+j*1024+512]) * (phi**(64*i) % q) % q
    ...
    for j in range(128):
        for i in range(256):
            a7[i+j*512]     = (a6[i+j*512] + a6[i+j*512+256]) % q
            a7[i+j*512+256] = (a6[i+j*512] - a6[i+j*512+256]) * (phi**(128*i) % q) % q
